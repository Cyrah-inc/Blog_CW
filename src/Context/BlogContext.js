...
export const BlogProvider = ({children}) => {
    ...
    const addBlogPost = async (title, content,image, callback) => {
        let result = await jsonserver.post('/blogposts', {title, content,image})
        console.log(result.status)
        // dispatch({type: 'add_blogpost', payload: {title, content}});
        result.status===201? callback() : null
    }
    const editBlogPost = async (id, title, content,image, callback)=>{
        
        
        let result = await jsonserver.put(`/blogposts/${id}`,{title, content, image})
